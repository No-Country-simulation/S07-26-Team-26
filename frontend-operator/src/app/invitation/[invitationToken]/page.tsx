...
import { Clock3, Building2, Briefcase, Megaphone, ArrowRight } from "lucide-react";
...
export default function InvitationPage() {
  ...
  const { data: invitation, isLoading, isError } = useInvitationQuery(invitationToken);
          ...
          <CardContent className="p-0">
            ...
            {isError && (
              <div className="flex flex-col items-center gap-3 text-center">
                <h1 className="text-lg font-semibold text-graphite-900">Invitación no encontrada</h1>
                <p className="text-sm text-graphite-500">
                  Este enlace ya no es válido o expiró. Solicita una nueva invitación por correo electrónico.
                </p>
              </div>
            )}
              ...
              <>
                <div className="mb-6 flex flex-col items-center text-center">
                  ...
                  <p className="mt-2 text-sm text-graphite-500">
                    Has sido invitado a completar el benchmark de infraestructura de Project Ghost Load.
                  </p>
                </div>

                <div className="space-y-3 rounded-md border border-graphite-100 bg-graphite-50/50 px-5 py-4">
                  <DetailRow icon={Building2} label="Empresa" value={invitation.companyName} />
                  <DetailRow icon={Briefcase} label="Cargo" value={invitation.role} />
                  <DetailRow icon={Megaphone} label="Campaña" value={invitation.campaignName} />
                  <DetailRow icon={Clock3} label="Tiempo estimado" value={`${invitation.estimatedMinutes} minutos`} />
                </div>
