...
import { PixelPdfBadge, PixelDocumentOpen, PixelMail, PixelExcelBadge } from "@/components/blocks/PixelIcons";
...
import { FOUNDER_CONTACT, REPORT_BRAND, buildReportReference } from "@/lib/brand";
...
export default function OperatorPdfPage() {
  ...
  function handleEmail() {
    const subject = encodeURIComponent("Reporte de Benchmark — Project Ghost Load");
  ...
  const isGenerating = reportStatus === "NOT_REQUESTED" || reportStatus === "REPORT_GENERATING";
    ...
    <div className="space-y-6">
      ...
      {isFailed && (
        <div className="flex flex-col items-center gap-3 rounded-lg border border-red-100 bg-red-50/60 px-8 py-16 text-center">
          ...
          <p className="max-w-sm text-sm text-graphite-500">
            Ocurrió un problema generando el documento. Vuelve a intentarlo en unos minutos.
          </p>
        </div>
      )}
        ...
        <>
          ...
          <div className="flex flex-col items-center gap-3 rounded-lg border border-forest-100 bg-forest-50/60 px-8 py-10 text-center print:hidden">
            ...
            <p className="max-w-md text-sm text-graphite-500">
              Puntaje {overallScore}/100 · Nivel {maturityLevel} · Percentil {percentile} de la industria.
              Descárgalo, compártelo o envíalo directamente a tu equipo.
            </p>
          </div>
            ...
            <Card className="overflow-hidden print:border-none print:shadow-none">
              ...
              <div className="bg-forest-950 px-8 py-6 text-forest-50">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold">{invitation?.companyName ?? "Tu empresa"}</p>
                      <p className="text-xs text-forest-50/50">
                        {REPORT_BRAND.reportTitle} — {REPORT_BRAND.documentLabel}
                      </p>
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    ...
                    <p className="mt-1 font-mono text-[10px] text-forest-50/40">Ref. {reportRef}</p>
                  </div>
                </div>
              </div>
              ...
              <CardContent className="space-y-6 p-8">
                ...
                <div className="flex flex-wrap items-center gap-8 border-b border-graphite-100 pb-6">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-graphite-400">Puntaje de Benchmark</p>
                    <p className="font-tabular text-4xl font-semibold text-forest-800">{overallScore}/100</p>
                  </div>
                  <span className={cn("rounded-sm border px-3 py-1 text-sm font-medium", maturityColor(maturityLevel))}>
                    {maturityLevel}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-x-8 gap-y-3 rounded-md border border-graphite-100 bg-graphite-50/50 px-5 py-4">
                    <div>
                      <p className="font-tabular text-2xl font-semibold text-graphite-900">Percentil {percentile}</p>
                      <p className="text-xs text-graphite-500">frente a los operadores evaluados en el benchmark</p>
                    </div>
                    <div className="flex items-center gap-1.5 text-sm">
                      ...
                      <span className="text-graphite-600">
                        <strong className="text-graphite-900">{aboveAverage ? "Por encima" : "Por debajo"}</strong> del
                ...
                <div>
                  ...
                  {kpis ? (
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                      <MiniKpi label="Capacidad Ociosa" value={`${formatNumber(kpis.idleCapacityKw)} kW`} />
                      <MiniKpi label="Capacidad Ociosa %" value={`${kpis.idleCapacityPct}%`} />
                      <MiniKpi label="Potencia / GPU" value={`${kpis.powerPerGpuKw} kW`} />
                      <MiniKpi label="Margen de Refrigeración" value={`${formatNumber(kpis.coolingHeadroomKw)} kW`} />
                      <MiniKpi label="Necesidad Proyectada" value={`${formatNumber(kpis.projectedCapacityNeedKw)} kW`} />
                    </div>
                  ) : (
                    <p className="text-sm text-graphite-400">Completa la calculadora para ver esta sección.</p>
                  )}
                </div>
                ...
                <div className="rounded-md border border-gold-100 bg-gold-50/40 p-5">
                  ...
                  <p className="text-sm text-graphite-700">
                    {FOUNDER_CONTACT.role} · Listos para conversar sobre lo que este reporte significa para tu roadmap.
                  </p>
                  <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-graphite-600">
                    ...
                    <span className="flex items-center gap-1.5">
                      <Phone className="h-3.5 w-3.5" /> {FOUNDER_CONTACT.phone}
                    </span>
                    <a
                ...
                <p className="border-t border-graphite-100 pt-4 text-center text-[11px] text-graphite-400">
                  {REPORT_BRAND.confidentialityNote} · Generado el {formatDate(generatedAt)} · Ref. {reportRef}
                </p>
              </CardContent>
            </Card>
          </div>
...
function MiniKpi({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-graphite-100 bg-graphite-50/50 px-4 py-3">
      ...
      <p className="font-tabular text-base font-semibold text-graphite-900">{value}</p>
    </div>
  );
}
